...
begin
  try
    for i := 0 to 25 do
    begin
      cmd  := Format('L%d:0'#10, [i]);
      ComPort1.WriteStr(cmd);

      LEDStateArray[i] := False;
      SetLEDDisplay(i, False);
...
begin
  try
    for i := 0 to 26 do
    begin
      cmd := Format('L%d:1'#10, [i]);
      ComPort1.WriteStr(cmd);

      LEDStateArray[i] := True;
      SetLEDDisplay(i, True);
...
begin
  ...
  while True do
  begin
    p := Pos(#10, RxBuffer); // cari newline
    if p = 0 then Break;

    line := Trim(Copy(RxBuffer, 1, p - 1));
    Delete(RxBuffer, 1, p);

    Log.Lines.Add('RX: ' + line);

    //===========TRIGER============
    //Format: T:1 atau T:0
    if Copy(line, 1, 2) = 'T:' then
    begin
      val := StrToIntDef(Copy(line, 3, MaxInt), -1);

      if val = 1 then
      begin
        // Trigger ON -> semua LED ON
        for index := 0 to NumberOfButton -1 do
        begin
          LEDStateArray[index] := True;
          SetLEDDisplay(index, True);
        end;
      end
      else
      begin
        //Trigger OFF -> matikan semua LED di tampilan Delphi
        for index := 0 to NumberOfButton - 1 do
        begin
          LEDStateArray[index] := False;
          SetLEDDisplay(index, False);
        end;
      end;
    end;

    //Format: S<index>:<value>
    if (Length(line) >= 3) and (line[1] = 'S') then
    begin
      p := Pos(':', line);
      if p > 0 then
      begin
        index := StrToIntDef(Copy(line, 2, p - 2), -1);
        val := StrToIntDef(Copy(line, p + 1, MaxInt), -1);

        if (index >= 0) and (index < NumberOfButton) then
        begin
          if val = 1 then
          begin
            //LED yang diterima ON
            LEDStateArray[index] := True;
            SetLEDDisplay(index, True);
            ...
            begin
              // index genap -> pasangan ganjil
              LEDStateArray[index + 1] := False;
              SetLEDDisplay(index + 1, False);
            end
            else
            begin
              // index ganjil -> pasangan genap
              LEDStateArray[index - 1] := False;
              SetLEDDisplay(index - 1, False);
            end;
          end;
...
begin
  ...
  LEDArray[8] := shapeLed09;
  LEDArray[9] := shapeLed10;
  LEDArray[10] := shapeLed11;
  LEDArray[11] := shapeLed12;
  LEDArray[12] := shapeLed13;
  LEDArray[13] := shapeLed14;
  LEDArray[14] := shapeLed15;
  LEDArray[15] := shapeLed16;
  LEDArray[16] := shapeLed17;
  LEDArray[17] := shapeLed18;
  LEDArray[18] := shapeLed19;
  LEDArray[19] := shapeLed20;
  LEDArray[20] := shapeLed21;
  LEDArray[21] := shapeLed22;
  LEDArray[22] := shapeLed23;
  LEDArray[23] := shapeLed24;
...
var
  ...
  cmd: string;
begin
  if not isComPortConnected then Exit;

  idx := TShape(Sender).Tag;

  if LEDStateArray[idx] then
    cmd := Format('L,%d,0'#10, [idx])
  else
    cmd := Format('L,%d,1'#10, [idx]);
