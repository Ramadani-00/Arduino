...
{  
  ...
  // LED 1 (Pin 2) langsung menyala terus sejak awal start
...
void loop()
{
  ...
  if(Serial.available()>0)
  {
    ...
    if (true==receiveWithStartEndMarkers(caracter, receivedCMD))
    {
      ...
      // Mengunci LED 1 agar tidak bisa dimatikan lewat serial individu
      if (cmd_code==LCN_CODE && cmd_number != 0)
      {
        ledAction(cmd_number, cmd_state);        
      }
      else 
      if (cmd_code==LCA_CODE)
      {
        if (255==cmd_number) 
        {          
          ledAllAction(cmd_state);
          // Memastikan LED 1 kembali menyala jika Delphi mengirim perintah "All"
          digitalWrite(ledPin[0], LED_ON);
          stateLed[0] = LED_ON;
        }
      }
  ...
  // 2. KONTROL REAL-TIME SWITCH MENGIKAT LED & SINKRONISASI DELPHI
  ...
  if (digitalRead(buttonPin[0]) == BUTTON_PRESSED) {
    if (stateLed[1] != LED_ON) {
      ...
      build_send_command(sentCMD, LSN_CODE, 1, LED_ON);
      Serial.write(sentCMD, sizeof(sentCMD));
    }
  } else {
    if (stateLed[1] != LED_OFF) {
      ...
      build_send_command(sentCMD, LSN_CODE, 1, LED_OFF);
      Serial.write(sentCMD, sizeof(sentCMD));
    }
  }
  ...
  if (digitalRead(buttonPin[1]) == BUTTON_PRESSED) {
    if (stateLed[2] != LED_ON) {
      ...
      build_send_command(sentCMD, LSN_CODE, 2, LED_ON);
      Serial.write(sentCMD, sizeof(sentCMD));
    }
  } else {
    if (stateLed[2] != LED_OFF) {
      ...
      build_send_command(sentCMD, LSN_CODE, 2, LED_OFF);
      Serial.write(sentCMD, sizeof(sentCMD));
    }
  }
