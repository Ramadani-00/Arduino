...
begin
  ...
  begin
    ...
    Log.Lines.Add('RX LINE: ' + line);

    //Format: S<index>:<value>
    if (Length(line) >= 4) and (line[1] = 'S') then
    begin
      p := Pos(':', line);
      if p > 0 then
      begin
  ...
  index := StrToIntDef(Copy(s, 2, - 2), -1);
  val := StrToIntDef(Copy(s, p + 1, MaxInt), -1);
